...
void DataContainer::append(QStringList listA, QStringList listB)
{
    ...
    // Based on other functions, the lists should always be the same size.  However, if they have ended up of different sizes, we take the smallest.
...
void DataContainer::importMask()
{
    ...
    if (!tempData->isNull())
    {
        ...
        maskCreated = true;
        // If a mask is loaded, it is assumed that the user wants to use the mask so it we set this in the global Settings object
