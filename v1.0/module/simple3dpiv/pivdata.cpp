...
PivData::PivData(int widthPass, int heightPass, int lengthPass)
{
    _width = widthPass;
    _height = heightPass;
    _length = lengthPass;
...
void PivData::createGrids()
{
    ...
    grid = new PivPointData [_width*_height*_length + 1];
    gridsCreated = true;
}
...
void PivData::initializeToZero()
{
    ...
    if (gridsCreated)
    {
        ...
        for (i = 0; i < _height; i++)
        {
            ...
            {
                for (k = 0; k < _length; k++)
                    grid[i*_width*_length + j*_length + k] = oneToZero();
            }
        }
    }
}
...
int PivData::numValid()
{
    ...
    for (i = 0; i < _height; i++)
    {
        ...
        {
            for (k = 0; k < _length; k++)
                if (grid[i*_width*_length + j*_length + k].valid)
                    count++;
        }
    }
...
//
void PivData::read(int indexPass, QString filename)
{
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        while(!(in.isNull()))
        {
            ...
            pointData.y = in.section(" ",1,1).toDouble();
            // Reads in second column: z
            pointData.z = in.section(" ",2,2).toDouble();
            // Reads in third column: u
            pointData.u = in.section(" ",3,3).toDouble();
            // Reads in fourth column: v
            pointData.v = in.section(" ",4,4).toDouble();
            // Reads in fourth column: w
            pointData.w = in.section(" ",5,5).toDouble();
            // Reads in fifth column: signal-to-noise ratio
            pointData.snr = in.section(" ",6,6).toDouble();
            // Reads in sixth column: validity of the point
            //if (in.section(" ",7,7).toInt() == 1)
            //    pointData.valid = true;
            //else
                pointData.valid = true;
            // Reads in seventh column: if the point has been filtered or not
            //if (in.section(" ",7,7).toInt() == 1)
            //    pointData.filtered = true;
            //else
                pointData.filtered = false;
            // Reads in the eigth column: average image intensity of the interrogation window
            pointData.intensity = in.section("\t",8,8).toDouble();
        ...
        setList(pointList);
        qDebug() << "read_end" << endl << this->list().length();
    }
    // Do some proper debugging here at some point...
    else
        qDebug() << "Problem reading file: " << filename.toLocal8Bit().data() << endl;
}

//
void PivData::toGrids(QList<PivPointData> _data)
{
    int i, j, k, m, removeX, removeY, removeZ;
    ...
    for (i = 0; i < _data.size(); i++)
    {
        ...
        zVal = _data.value(i).z;
        xCount = 0; yCount = 0; zCount = 0;
        /* Check to see how many unique values of x there are and create a list
           of these values */
        for (j = 0; j < xList.size(); j++)
            if (xVal == xList.value(j))
                xCount++;
        if (xCount == 0)
            xList.append(xVal);
        /* Check to see how many unique values of y there are and create a list
           of these values */
        for (j = 0; j < yList.size(); j++)
            if (yVal == yList.value(j))
                yCount++;
        if (yCount == 0)
            yList.append(yVal);
        /* Check to see how many unique values of z there are and create a list
           of these values */
        for (j = 0; j < zList.size(); j++)
            if (zVal == zList.value(j))
                zCount++;
        if (zCount == 0)
            zList.append(zVal);
    }
    ...
    _height = yList.size();
    // Height is defined by the number of unique z values
    _length = zList.size();

    xTemp = new double [_width + 1];
    yTemp = new double [_height + 1];
    zTemp = new double [_length + 1];
    ...
    for (k = 0; k < _length; k++)
    {
        // Putting z values into the grid
        minZ = 100000000.0;
        for (m = 0; m < zList.size(); m++)
        {
            if (zList.value(m) < minZ)
            {
                minZ = zList.value(m);
                removeZ = m;
            }
            ...
            for (j = 0 ; j < _width; j++)
            {
                ...
                while (m < _data.size() && !found)
                {
                    /* If there is a data point in the list that corresponds to the
                        gridded value */
                    if (xVal == _data.value(m).x && yVal == _data.value(m).y && zVal == _data.value(m).z)
                    {
                        grid[_width*_length*i + j*_length + k].u = _data.value(m).u;
                        grid[_width*_length*i + j*_length + k].v = _data.value(m).v;
                        grid[_width*_length*i + j*_length + k].w = _data.value(m).w;
                        grid[_width*_length*i + j*_length + k].x = xVal;
                        grid[_width*_length*i + j*_length + k].y = yVal;
                        grid[_width*_length*i + j*_length + k].z = zVal;
                        grid[_width*_length*i + j*_length + k].snr = _data.value(m).snr;
                        grid[_width*_length*i + j*_length + k].valid = true;
                        grid[_width*_length*i + j*_length + k].filtered = _data.value(m).filtered;
                        grid[_width*_length*i + j*_length + k].intensity = _data.value(m).intensity;
                        found = true;
                    }
                    // If not, make the point blank and invalid
                    else
                    {
                        grid[_width*_length*i + j*_length + k].u = 0.0;
                        grid[_width*_length*i + j*_length + k].v = 0.0;
                        grid[_width*_length*i + j*_length + k].w = 0.0;
                        grid[_width*_length*i + j*_length + k].x = xVal;
                        grid[_width*_length*i + j*_length + k].y = yVal;
                        grid[_width*_length*i + j*_length + k].z = zVal;
                        grid[_width*_length*i + j*_length + k].snr = 0.0;
                        grid[_width*_length*i + j*_length + k].valid = false;
                        grid[_width*_length*i + j*_length + k].filtered = false;
                        grid[_width*_length*i + j*_length + k].intensity = 0.0;
                    }
                    m++;
                }
            }
...
PivPointData PivData::data(int i, int j, int k)
{
    PivPointData pointData;
    if (gridsCreated && i >= 0 && i < _height && j >= 0 && j < _width
            && k >= 0 && k < _length)
    {
        pointData = grid[i* _width* _length + j * _length + k];
    }
    else
    {
        pointData = oneToZero();
...
void PivData::setData(int i, int j, int k, PivPointData dataPass)
{
    if (gridsCreated && i >= 0 && i < _height && j >= 0 && j < _width
            && k >= 0 && k < _length)
    {
        grid[i* _width* _length + j * _length + k] = dataPass;
    }
}

/* ---------- Validity ---------*/
bool PivData::isValid(int i, int j, int k)
{
    //qDebug() << gridsCreated;
    if (gridsCreated && i >= 0 && i < _height && j >= 0 && j < _width
            && k >= 0 && k < _length)
    {
        return grid[i* _width* _length + j * _length + k].valid;
        qDebug() << "valid:" << i << "," << j << "," << k;
    }
    else
        return false;
...
///*------- Filtering -------------*/
bool PivData::filtered(int i, int j, int k)
{
    if (gridsCreated && i >= 0 && i < _height && j >= 0 && j < _width
            && k >= 0 && k < _length)
    {
        return grid[i* _width* _length + j * _length + k].filtered;
    }
    else return false;
}

void PivData::setFilter(int i, int j, int k, bool filterPass)
{
    if (gridsCreated && i >= 0 && i < _height && j >= 0 && j < _width
            && k >= 0 && k < _length)
    {
        grid[i* _width* _length + j * _length + k].filtered = filterPass;
    }
}
...
void PivData::saveToFile(QString filename)
{
    ...
    out << "TITLE = \"B00001\"\n";
    out << "VARIABLES = \"x\", \"y\", \"z\", \"Vx\", \"Vy\", \"Vz\", \"NCC\"\n";
    out << "ZONE T=\"Frame 0\", I=   , J=   , K=   \n";
    //out << "GRID LENGTH = 20, WIN_DELTA = 12 CAND_WIN_R = 20\n";
    for (int i = 0; i < this->list().size(); i++)
    {
        PivPointData haha= this->list().at(i);
        //out << "":(x,y,z)=(" << (*iter)->xpos << "," << (*iter)->ypos << "," << (*iter)->zpos << "),\tindex:"
        //    << index((*iter)->xpos,(*iter)->ypos,(*iter)->zpos) << "\n";

        out  << haha.x << " " << haha.y << " " << haha.z << " " <<
                     haha.u << " " << haha.v << " " << haha.w << " " << haha.ncc << "\n";
