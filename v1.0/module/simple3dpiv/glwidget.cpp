...
void GLWidget::draw3DCoordinate()
{
    ...
    // 画坐标格
    int i, j;
    for (i = 0; i < ySpan; i += yGridLength)
        for (j = 0; j < zSpan; j += zGridLength)
        {
            glBegin(GL_LINES);
    ...
    for (i = 0; i < xSpan; i += xGridLength)
        for (j = 0; j < ySpan; j += yGridLength)
        {
            glBegin(GL_LINES);
...
void GLWidget::drawVectors(PivData *pivData)
{
    ...
    qDebug() << "LENGTH=" << pivData->list().length();
    ...
    if (settings != NULL)
    {
        _intLengthX = settings->intLengthX();
        _intLengthY = settings->intLengthY();
        _intLengthZ = settings->intLengthZ();
    }
    ...
    if (!vectorGroupCreated)
        ...
        //createVectorGroup(pivData->list().length());
...
void GLWidget::setSettings(Settings * settingsPass)
{
    if (settingsPass != NULL)
    {
        this->settings = settingsPass;
        this->xSpan = settings->getROIWidth();
        this->ySpan = settings->getROIHeight();
        this->zSpan = settings->getROIDepth();
        ...
        this->xGridLength = settings->intLengthX();
        this->yGridLength = settings->intLengthY();
        this->zGridLength = settings->intLengthZ();
    }
}
